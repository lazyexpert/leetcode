...
survives a render. Running this produces deterministic output regardless
of the caller's timezone (all dates are UTC).
...
Usage:
    ./render.py                           # write directly to repo root
    ./render.py --db /tmp/scratch.db --out-dir /tmp/rendered
"""
...
def render_timings(conn: sqlite3.Connection) -> str:
    ...
    header = [
        '# Solution Timings',
        '',
        'Time from `/leetcode-new` scaffold to `/leetcode-done` commit, in minutes.',
